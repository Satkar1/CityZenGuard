...
import {
  ...
  type Fir, type InsertFir,
  type ChatMessage, type InsertChatMessage,
  type Notification, type InsertNotification,
  users, cases, firs, chatMessages, notifications
} from "@shared/schema";
...
export class MemStorage implements IStorage {
  ...
  private firs = new Map<string, Fir>();
  ...
  async getCasesByUserId(userId: string): Promise<Case[]> {
    return Array.from(this.cases.values()).filter((caseItem) => caseItem.citizenId === userId);
  }

  async createCase(insertCase: InsertCase): Promise<Case> {
    const id = randomUUID();
    const caseItem: Case = { ...insertCase, id, createdAt: new Date(), filedDate: new Date() };
    this.cases.set(id, caseItem);
    return caseItem;
  }

  async getFirsByOfficerId(officerId: string): Promise<Fir[]> {
    return Array.from(this.firs.values()).filter((fir) => fir.officerId === officerId);
  }
